...
const PatternsDocx = (patterns, ds, user) => {
	...
	for (let i = 0; i < 7; i++) {
		const items = patterns.filter((x) => x.ts.includes(date.format('YYYY-MM-DD')));
		if (items.length > 0) {
			doc.createParagraph(date.format('DD MMMM, dddd')).heading1();
			let n = 0;
			items.forEach((pattern) => {
				const { lesson, ts, dur } = pattern;
	...
	packer.toBlob(doc).then((blob) => {
		saveAs(blob, `Pattern - ${user} ${date.add(-7, 'day').format('DD MMMM')}.docx`);
	});
};

export default PatternsDocx;
